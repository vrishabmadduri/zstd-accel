...
class L1MemHelperModule(outer: L1MemHelper, printInfo: String = "", queueRequests: Boolean = false, queueResponses: Boolean = false)(implicit p: Parameters) extends LazyModuleImp(outer)
  ...
  when (io.status.valid) {
    rleLogger.logInfo(printInfo + " setting status.dprv to: %x compare %x\n", io.status.bits.dprv, UInt(PRV.M))
  ...
  when (tags_init_reg =/= (outer.numOutstandingRequestsAllowed).U) {
    ...
    when (tags_for_issue_Q.io.enq.ready) {
      rleLogger.logInfo(printInfo + " tags_for_issue_Q init with value %d\n", tags_for_issue_Q.io.enq.bits)
  ...
  when (dmem.a.fire()) {
    when (request_input.bits.cmd === M_XRD) {
      rleLogger.logInfo(printInfo + " L2IF: req(read) vaddr: 0x%x, paddr: 0x%x, wid: 0x%x, opnum: %d, sendtag: %d\n",
    ...
    when (request_input.bits.cmd === M_XWR) {
      rleLogger.logInfo(printInfo + " L2IF: req(write) vaddr: 0x%x, paddr: 0x%x, wid: 0x%x, data: 0x%x, opnum: %d, sendtag: %d\n",
        request_input.bits.addr,
        tlb.io.resp.paddr,
        request_input.bits.size,
  ...
  when (fire_actual_mem_resp.fire(tags_for_issue_Q.io.enq.ready) &&
    tags_for_issue_Q.io.enq.valid) {
      rleLogger.logInfo(printInfo + " tags_for_issue_Q add back tag %d\n", tags_for_issue_Q.io.enq.bits)
  }
  ...
  when (dmem.d.fire()) {
    when (edge.hasData(dmem.d.bits)) {
      rleLogger.logInfo(printInfo + " L2IF: resp(read) data: 0x%x, opnum: %d, gettag: %d\n",
        dmem.d.bits.data,
        global_memop_ackd,
        dmem.d.bits.source)
    } .otherwise {
      rleLogger.logInfo(printInfo + " L2IF: resp(write) opnum: %d, gettag: %d\n",
        global_memop_ackd,
        dmem.d.bits.source)
    }
  }
  ...
  when (response_output.fire()) {
    rleLogger.logInfo(printInfo + " L2IF: realresp() data: 0x%x, opnum: %d, gettag: %d\n",
      resultdata,
